...
export default function IntegratedPortfolio(): JSX.Element {
  ...
  const currentCaseStudy = CASE_STUDIES.find(c => c.id === selectedCaseStudy) || CASE_STUDIES[0];
              ...
              <div className="grid lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-2 bg-zinc-900/40 border border-zinc-800/80 p-8 rounded-3xl space-y-8 backdrop-blur-xl">
                  <div className="flex flex-wrap justify-between items-start gap-4 pb-6 border-b border-zinc-800">
                    <div>
                      <span className="text-xs font-mono text-yellow-500">{currentCaseStudy.category}</span>
                      <h2 className="text-2xl font-bold text-white mt-1">{currentCaseStudy.title}</h2>
                    </div>
                    <a href={currentCaseStudy.liveUrl} target="_blank" rel="noopener noreferrer" className="bg-yellow-500 hover:bg-yellow-400 text-black text-xs font-bold px-5 py-2.5 rounded-xl shadow-lg transition-all">Launch Production App 🚀</a>
                  </div>

                  <div className="grid grid-cols-3 gap-4 font-mono">
                    {Object.entries(currentCaseStudy.metrics).map(([k, v], idx) => (
                  ...
                  <div className="space-y-4 text-xs leading-relaxed">
                    <div>
                      ...
                      <p className="text-zinc-300 font-light">{currentCaseStudy.problem}</p>
                    </div>
                    <div>
                      <h4 className="font-mono text-zinc-400 uppercase tracking-wider mb-1">// Technical Solution</h4>
                      <p className="text-zinc-300 font-light">{currentCaseStudy.solution}</p>
                    </div>
                  </div>
                </div>
              </div>
